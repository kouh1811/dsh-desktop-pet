// dsh-desktop-pet 浏览器 half：纯 DOM 自渲染宠物层（A 模式——GUI 内悬浮宠物）。
...
import { STATE_PATH, INTERACT_PATH, CONFIG_PATH, ASSETS_PATH, EVENTS_PATH, COMPANION_LAUNCH_PATH } from '../src/routes.mjs'
...
const CSS = `
[data-dsh-desktop-pet] { position: fixed; right: 16px; bottom: 16px; z-index: 2147483000;
...
[data-dsh-desktop-pet] .pet-stage { position: relative; width: var(--pet-size, 110px); height: var(--pet-size, 110px); display: grid; place-items: center;
...
[data-dsh-desktop-pet] .pet-effects { position: absolute; left: 0; top: 0; width: var(--pet-size, 110px); height: var(--pet-size, 110px);
  pointer-events: none; overflow: visible; z-index: 2; }
[data-dsh-desktop-pet] .pet-hitarea { position: absolute; inset: 0; width: var(--pet-size, 110px); height: var(--pet-size, 110px);
  cursor: grab; touch-action: none; z-index: 3; border-radius: 8px; }
[data-dsh-desktop-pet] .pet-sprite { pointer-events: none; /* 视觉层：定位/尺寸/transform 由 JS 内联（宿主可能覆盖 CSS 注入） */ }
[data-dsh-desktop-pet] .pet-sprite.ready { display: block; }
/* 状态卡：默认置于宠物下方，间距足够（角色 bob 浮动 ±4px 不触到）+ 贴底时翻上方。 */
[data-dsh-desktop-pet] .pet-status { position: absolute; left: 50%; top: calc(100% + 18px); transform: translateX(-50%);
...
[data-dsh-desktop-pet] .pet-status::after { /* 连接尾：命中区覆盖宠物↔卡片间隙，hover 连续不闪断 */
...
[data-dsh-desktop-pet]:hover .pet-status,
[data-dsh-desktop-pet]:focus-within .pet-status {
  opacity: 1; visibility: visible; pointer-events: auto;
  transform: translateX(-50%) translateY(0);
  transition: opacity .2s cubic-bezier(.16,1,.3,1), transform .2s cubic-bezier(.16,1,.3,1), visibility 0s;
  transition-delay: .06s; }
[data-dsh-desktop-pet] .pet-meta { display: flex; align-items: center; justify-content: space-between; gap: 8px; }
[data-dsh-desktop-pet] .pet-lv { background: rgba(86,134,254,.16); color: #B7C8FE; border-radius: 5px;
  padding: 2px 6px; font-size: 10px; font-weight: 600; line-height: 16px; white-space: nowrap; }
[data-dsh-desktop-pet] .pet-stats { color: #E8EBF2; font-size: 11px; line-height: 16px;
  font-variant-numeric: tabular-nums; white-space: nowrap; }
[data-dsh-desktop-pet] .pet-note { color: #E8EBF2; font-size: 11px; line-height: 15px;
  text-overflow: ellipsis; overflow: hidden; white-space: nowrap; }
[data-dsh-desktop-pet] .pet-status::after { /* 连接尾：命中区覆盖宠物↔卡片间隙，hover 连续不闪断（main 定位由 JS 内联） */
  ...
  border-top: 1px solid rgba(255,255,255,.10); border-left: 1px solid rgba(255,255,255,.10);
  border-top-left-radius: 3px; pointer-events: auto; }
[data-dsh-desktop-pet] .pet-status.pet-status-above::after { top: auto; bottom: auto; top: -5px; } /* 贴底翻转：卡在上方，连接尾朝下指向角色 */
[data-dsh-desktop-pet] .pet-menu { display: none; position: absolute; left: 50%; top: calc(100% + 12px); transform: translateX(-50%);
  width: max-content; gap: 6px; padding: 6px; border-radius: 8px;
  background: rgba(20,20,28,.72); }
[data-dsh-desktop-pet] .pet-bubble { position: absolute; left: 50%; top: calc(100% + 12px); transform: translateX(-50%);
  background: rgba(24,28,38,.94); color: #E8EBF2; font-size: 11px; padding: 4px 8px; border-radius: 10px;
  white-space: nowrap; pointer-events: none; animation: dsh-desktop-pet-pop .25s ease-out;
  z-index: 3; }
[data-dsh-desktop-pet] .pet-menu.open { display: flex; }
[data-dsh-desktop-pet] .pet-menu button { flex: 1; border: 0; border-radius: 6px; padding: 4px 8px;
  font-size: 11px; cursor: pointer; background: rgba(255,255,255,.14); color: #E8EBF2; }
[data-dsh-desktop-pet] .pet-menu button:hover { background: rgba(255,255,255,.28); }
[data-dsh-desktop-pet] .pet-heart { position: absolute; font-size: 20px; pointer-events: none;
  animation: dsh-desktop-pet-float 1.8s ease-out forwards; }
...
[data-dsh-desktop-pet] .pet-stage.pet-motion-bob { animation: dsh-desktop-pet-m-bob 2.4s ease-in-out infinite; }
[data-dsh-desktop-pet] .pet-stage.pet-motion-wiggle { animation: dsh-desktop-pet-m-wiggle .9s ease-in-out infinite; }
[data-dsh-desktop-pet] .pet-stage.pet-motion-squash { animation: dsh-desktop-pet-m-squash .7s ease-in-out infinite; }
[data-dsh-desktop-pet] .pet-stage.pet-motion-shake { animation: dsh-desktop-pet-m-shake .3s linear infinite; }
[data-dsh-desktop-pet] .pet-stage.pet-motion-sigh { animation: dsh-desktop-pet-m-sigh 1.6s ease-in-out infinite; }
[data-dsh-desktop-pet] .pet-stage.pet-motion-hop { animation: dsh-desktop-pet-m-hop .6s ease-in-out infinite; }
[data-dsh-desktop-pet] .pet-stage.pet-motion-tilt { animation: dsh-desktop-pet-m-tilt 1.2s ease-in-out infinite; }
[data-dsh-desktop-pet] .pet-stage.pet-motion-float { animation: dsh-desktop-pet-m-float 3.2s ease-in-out infinite; }
[data-dsh-desktop-pet] .pet-stage.pet-motion-wave { animation: dsh-desktop-pet-m-wave 1s ease-in-out infinite; }
@keyframes dsh-desktop-pet-m-bob { 0%,100% { transform: translateY(0); } 30% { transform: translateY(-3px); } 60% { transform: translateY(-4px); } }
@keyframes dsh-desktop-pet-m-wiggle { 0%,100% { transform: rotate(0); } 25% { transform: rotate(-2deg); } 75% { transform: rotate(2deg); } }
@keyframes dsh-desktop-pet-m-squash { 0%,100% { transform: scale(1,1); } 25% { transform: scale(1.06,.94); } 50% { transform: scale(.96,1.04); } 75% { transform: scale(1.03,.97); } }
@keyframes dsh-desktop-pet-m-shake { 0%,100% { transform: translateX(0); } 30% { transform: translateX(-2px); } 60% { transform: translateX(2px); } 80% { transform: translateX(-1px); } }
@keyframes dsh-desktop-pet-m-sigh { 0%,100% { transform: translateY(0) scale(1,1); } 40% { transform: translateY(1.5px) scale(1,.98); } }
@keyframes dsh-desktop-pet-m-hop { 0%,100% { transform: translateY(0); } 40% { transform: translateY(-6px); } 70% { transform: translateY(0); } }
@keyframes dsh-desktop-pet-m-tilt { 0%,100% { transform: rotate(0); } 30% { transform: rotate(-4deg); } 70% { transform: rotate(4deg); } }
@keyframes dsh-desktop-pet-m-float { 0%,100% { transform: translateY(0); } 50% { transform: translateY(-3px); } }
@keyframes dsh-desktop-pet-m-wave { 0%,100% { transform: rotate(0); } 20% { transform: rotate(-6deg); } 40% { transform: rotate(6deg); } 60% { transform: rotate(-4deg); } 80% { transform: rotate(4deg); } }
@keyframes dsh-desktop-pet-float { 0% { opacity: 1; transform: translateY(0) scale(.7); }
  70% { opacity: 1; }
  100% { opacity: 0; transform: translateY(-72px) scale(1.25); } }
@keyframes dsh-desktop-pet-pop { from { opacity: 0; transform: translateX(-50%) translateY(4px); } }
[data-dsh-desktop-pet][data-dsh-desktop-pet-inert] { opacity: .25; pointer-events: none; }
[data-dsh-desktop-pet][data-dsh-desktop-pet-hidden] { display: none; }
[data-dsh-desktop-pet] .pet-stage:focus-visible { outline: 2px solid rgba(255,255,255,.6); outline-offset: 2px; border-radius: 8px; }
@media (prefers-reduced-motion: reduce) {
  [data-dsh-desktop-pet] .pet-stage { animation: none !important; }
  [data-dsh-desktop-pet] .pet-sprite { animation: none !important; }
  [data-dsh-desktop-pet] .pet-heart { animation: none; opacity: 0; }
  [data-dsh-desktop-pet] .pet-bubble { animation: none; }
  [data-dsh-desktop-pet] .pet-status { transition: none !important; }
}
`

export function apply(ctx = {}) {
  // 幂等守卫：bundle 重复执行（dev/HMR 重建、loader 重跑）时不双宠物双 style。
  if (document.querySelector('[data-dsh-desktop-pet]') !== null) {
    console.warn('[dsh-desktop-pet] apply 已存在实例，跳过重复挂载')
    return () => {}
  }
  ...
  host.setAttribute('data-dsh-desktop-pet', '')
  ...
  desktopBtn.textContent = '🖥️ 桌面模式'
  desktopBtn.style.cssText = BTN_STYLE
  menu.append(feedBtn, playBtn, roleBtn, desktopBtn)
  ...
  const showPlaceholder = (name) => {
    ...
    console.warn(`[dsh-desktop-pet] 状态 ${name} 缺少可用 sheet（manifest 应含全部 15 状态；若已声明则素材加载失败）`)
  }
  ...
  const loadAssets = async (attempt = 1) => {
    try {
      ...
      const pref = (() => { try { return localStorage.getItem('dsh-desktop-pet:character') ?? null } catch { return null } })()
  ...
  const switchCharacter = async (id) => {
    ...
    try {
      ...
      try { localStorage.setItem('dsh-desktop-pet:character', id) } catch { /* 隐私模式忽略 */ }
      transient = null
      transientUntil = 0
      joyUntil = 0
  ...
  const tick = () => {
    ...
    if (cfg && loaded.has(sheetKey(cfg.sheet))) {
      ...
      if (cfg.playback !== undefined && !['loop', 'pingpong', 'once', 'blink'].includes(cfg.playback)) {
        console.warn(`[dsh-desktop-pet] 状态 ${animState} playback "${cfg.playback}" 非法，按 loop 播放`)
      }
      const size = sheetSize.get(sheetKey(cfg.sheet))
      const frameW = size.w / cfg.frames
      if (!showingSprite) {
  ...
  const spawnHearts = () => {
    for (let i = 0; i < 4; i++) {
      ...
      // 此前 dsh-desktop-pet-float keyframes 失效导致爱心静态）。上浮 + 放大 + 淡出。
  ...
  const POS_KEY = 'dsh-desktop-pet:pos'
  const savePos = () => {
    try {
  ...
  // 桌面模式：请求 Node half 启动桌面伴侣（spawn 独立进程）。伴侣在场（心跳窗口）期间
  // 网页端宠物自动隐藏（/state companionOnline），退出伴侣后自动恢复。
  desktopBtn.addEventListener('click', async () => {
    toggleMenu(false)
    try {
      const res = await fetch(COMPANION_LAUNCH_PATH, {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
      ...
      const body = await res.json().catch(() => null)
      showReply(body?.alreadyRunning ? '桌面宠物已在运行' : '已启动桌面宠物 🐳')
    } catch {
      showReply('桌面宠物启动失败')
    }
  })
  ...
  //   dsh-desktop-pet:say    { text }          → 气泡说话
  //   dsh-desktop-pet:fx     { type: 'hearts' } → 爱心爆发
  //   dsh-desktop-pet:status { text }          → 状态卡 note 覆盖（临时，2.5s 恢复）
  // 派发方式：window.dispatchEvent(new CustomEvent('dsh-desktop-pet:say', { detail: { text } }))
  ...
  document.addEventListener('dsh-desktop-pet:say', onPetSay)
  document.addEventListener('dsh-desktop-pet:fx', onPetFx)
  document.addEventListener('dsh-desktop-pet:status', onPetStatus)
  ...
  const syncInert = () => {
    ...
    if (next !== pageHidden) {
      ...
      if (next === 'onboarding' || next === 'companion') {
        host.setAttribute('data-dsh-desktop-pet-hidden', '')
        host.removeAttribute('data-dsh-desktop-pet-inert')
        // 内联是权威（CSS 规则可能被宿主清理——属性设了但视觉不变）。
        host.style.display = 'none'
        host.style.opacity = ''
      } else if (next === 'dialog') {
        host.removeAttribute('data-dsh-desktop-pet-hidden')
        host.style.display = ''
        host.setAttribute('data-dsh-desktop-pet-inert', '')
  ...
  const dispose = () => {
    ...
    document.removeEventListener('dsh-desktop-pet:say', onPetSay)
    document.removeEventListener('dsh-desktop-pet:fx', onPetFx)
    document.removeEventListener('dsh-desktop-pet:status', onPetStatus)
