...
// lib/src/routes.mjs
var ROUTE_PREFIX = "/dsh-desktop-pet";
...
var COMPANION_LAUNCH_PATH = `${ROUTE_PREFIX}/companion/launch`;
...
function apply(ctx = {}) {
  ...
  desktopBtn.textContent = "\u{1F5A5}\uFE0F \u684C\u9762\u6A21\u5F0F";
  ...
  let manifest = { states: {} };
  let character = { id: "dsh-desktop-pet", states: {} };
  ...
  const showPlaceholder = (name2) => {
    ...
    console.warn(`[dsh-desktop-pet] \u72B6\u6001 ${name2} \u7F3A\u5C11\u53EF\u7528 sheet\uFF08manifest \u5E94\u542B\u5168\u90E8 15 \u72B6\u6001\uFF1B\u82E5\u5DF2\u58F0\u660E\u5219\u7D20\u6750\u52A0\u8F7D\u5931\u8D25\uFF09`);
  };
  ...
  const loadAssets = async (attempt = 1) => {
    try {
      ...
      const pref = (() => {
        try {
          return localStorage.getItem("dsh-desktop-pet:character") ?? null;
        } catch {
          return null;
        }
      })();
  ...
  const switchCharacter = async (id) => {
    ...
    try {
      ...
      try {
        localStorage.setItem("dsh-desktop-pet:character", id);
      } catch {
      }
      transient = null;
      transientUntil = 0;
  ...
  const tick = () => {
    ...
    if (cfg2 && loaded.has(sheetKey(cfg2.sheet))) {
      if (cfg2.playback !== void 0 && !["loop", "pingpong", "once", "blink"].includes(cfg2.playback)) {
        console.warn(`[dsh-desktop-pet] \u72B6\u6001 ${animState} playback "${cfg2.playback}" \u975E\u6CD5\uFF0C\u6309 loop \u64AD\u653E`);
      }
      const size = sheetSize.get(sheetKey(cfg2.sheet));
      const frameW = size.w / cfg2.frames;
      if (!showingSprite) {
