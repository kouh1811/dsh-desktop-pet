// dsh-desktop-pet Node half：积累型账本宿主 + assets 静态服务 + 活动/事件推导 + 状态持久化。
...
// 持久化：状态存 <dshHome>/data/dsh-desktop-pet/state.json（.tmp + rename 原子写，1s 防抖，
...
import { STATE_PATH, INTERACT_PATH, CONFIG_PATH, ROUTE_PREFIX, EVENTS_PATH, PRESENCE_PATH, SESSIONS_PATH, COMPANION_LAUNCH_PATH } from './src/routes.mjs'
export { STATE_PATH, INTERACT_PATH, CONFIG_PATH, ROUTE_PREFIX, EVENTS_PATH, PRESENCE_PATH, SESSIONS_PATH, COMPANION_LAUNCH_PATH }
...
/** 状态文件：<dshHome>/data/dsh-desktop-pet/state.json（不放插件目录——uninstall 会删）。 */
const DSH_HOME = process.env.DSH_HOME ?? resolve(import.meta.dirname, '../../..')
const STATE_FILE = join(DSH_HOME, 'data', 'dsh-desktop-pet', 'state.json')
...
export function apply(ctx) {
  ...
  ctx.effect(() => {
    const disposers = [
      // pet 服务（开放性窄缝）：只读快照 + 信号订阅。其他插件 inject ['dsh-desktop-pet.pet']
      // 消费；服务缺席时消费方应容忍（ dsh-desktop-pet 自己处理 sessions 缺席即先例）。
      // 命名带 dsh-desktop-pet 前缀避免与第三方插件的通用服务名（如 'pet'）撞名——
      // cordis 同 scope 同名服务直接抛 "service ... has been registered"（见
      // 2026-08-15-pet-service-namespace 决策记录）。
      // 不暴露任何写面（账本语义由 dsh-desktop-pet 独占，防第三方破坏积累不变量）。
      ctx.provide('dsh-desktop-pet.pet', {
      ...
      // ---- 桌面伴侣启动（显示层写面）：客户端「桌面模式」按钮触发 ----
      // spawn 伴侣进程（detached+unref：独立于 web 存活）；伴侣随后经 /presence 心跳，
      // 在场期间网页端宠物自动隐藏（/state companionOnline）。已在场时直接返回不重复拉起。
      // 二进制位于插件目录 companion/dsh-desktop-pet（link 安装时随插件目录走）。
      // 安全面与 /interact 同级：跨源校验 + body 上限。origin 缺省时伴侣用默认 GUI 源。
      webServer.register({
        kind: 'exact',
        path: COMPANION_LAUNCH_PATH,
        handler: async (req, res) => {
          try {
            if (req.method !== 'POST') {
              json(res, 405, { error: 'method not allowed; use POST' }, { allow: 'POST' })
            ...
            if (companionOnline(companionUntil, Date.now())) {
              // 心跳窗口在场不代表进程活着（强杀/崩溃时窗口有 45s 空窗）。
              // 以真实进程为准：有活进程才算 alreadyRunning，否则照常拉起。
              const alive = await new Promise((resolve) => {
                execFile('pgrep', ['-f', 'dsh-desktop-pet'], (err, stdout) => {
                  resolve(err === null && stdout.trim().length > 0)
                })
              })
              if (alive) {
                json(res, 200, { ok: true, alreadyRunning: true }, { 'cache-control': 'no-store' })
                return
              }
              // 进程已死：清掉残留心跳窗口，重新拉起（网页端宠物即刻恢复在场逻辑）
              companionUntil = pokePresence(companionUntil, Date.now(), false)
            }
            const bin = join(import.meta.dirname, '..', 'companion', 'dsh-desktop-pet')
            if (!existsSync(bin)) {
              json(res, 500, { error: 'companion binary not found' })
              return
            }
            const origin = typeof body.origin === 'string' && body.origin.length > 0 ? body.origin : null
            try {
              const args = origin !== null ? ['--base', origin] : []
              const child = spawn(bin, args, { detached: true, stdio: 'ignore' })
              // spawn 失败（EACCES 等）异步经 error 事件上报：拒绝时仍回 200（伴侣可能
              // 未启动但心跳会自然过期，网页端宠物照常），错误事件仅记日志。
              child.on('error', (error) => {
                console.error('[dsh-desktop-pet] companion spawn failed:', error.message)
              })
              child.unref()
  ...
  }, 'dsh-desktop-pet: state/interact/config/assets/ui routes + events')
}
